"""
Daily Outstanding Shipment Processor (WAWI / Packing List / UPS prep)
======================================================================
Encodes the SOP "001-SOP_Upload_Daily_Shipments_WAWI_PL_UPS" plus the
clarified business rules agreed through 2026-08-26. Runs entirely
locally - no internet, no AI calls, no cost.
...
explicitly:
    ...
    python process_shipments.py "C:\\Users\\you\\Downloads\\OS_2026-08-27.xlsx"

OUTPUT
------
    <same filename as source>_for_wawi.xlsx
saved to the same Downloads folder (e.g. OS_2026-08-27.xlsx ->
OS_2026-08-27_for_wawi.xlsx). Adds one new column, "Notes and
...
PRODUCT_SUBSTITUTIONS = {
    # "Amezcua Chi Pendant 3": {"ProdName": "Amezcua Chi Pendant 4", "ProdCode": "1310001029"},
}
...
def find_input_file(explicit_path: str | None) -> Path:
    ...
    if len(matches) == 0:
        ...
        print('  python process_shipments.py "C:\\path\\to\\file.xlsx"')
        print("=" * 70)
        sys.exit(1)
    ...
    print(f'  python process_shipments.py "{matches[-1]}"')
    print("=" * 70)
    sys.exit(1)
...
def output_path_for(src: Path) -> Path:
    """Same filename as the source, with '_for_wawi' appended before the
    extension (e.g. OS_2026-08-27.xlsx -> OS_2026-08-27_for_wawi.xlsx) -
    whatever date/text is in the name, left untouched."""
    return DOWNLOADS_DIR / f"{src.stem}_for_wawi{src.suffix}"
...
def main():
    ...
    print("Review the file, then upload it to WAWI manually as usual.")
